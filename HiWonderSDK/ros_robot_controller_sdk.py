...
class PacketFunction(enum.IntEnum):
    # Control functions available through the serial port
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED(LED control)
    PACKET_FUNC_BUZZER = 2  # buzzer control
    PACKET_FUNC_MOTOR = 3  # motor control
    PACKET_FUNC_PWM_SERVO = 4  # PWM, 1-4(PWM servo control, the board is numbered 1 to 4 from inside to outside)
    PACKET_FUNC_BUS_SERVO = 5  # bus servo control
    PACKET_FUNC_KEY = 6  # key obtain
    PACKET_FUNC_IMU = 7  # IMU(IMU obtain)
    PACKET_FUNC_GAMEPAD = 8  # handle grip obtain
    PACKET_FUNC_SBUS = 9  # remote control for aircraft model obtain
    PACKET_FUNC_OLED = 10 # OLED (OLED display content setting)
    PACKET_FUNC_RGB = 11 # RGB(set RGB color)
    PACKET_FUNC_NONE = 12
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # Sub-command 0x01 sets the SSID. The second byte is the string length, including the '\0' terminator.
